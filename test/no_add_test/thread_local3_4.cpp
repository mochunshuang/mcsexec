...
struct Manager
{
    ...
    // TODO(mcs):
    // 编译期idea提示 block 未初始化。 如何设计，故意明显的指明就是不初始化的
    // 外部 placement new ？ 还是有什么更拥有像 std::move那种语义强的组件明显指明
    struct Chunk
    {
    ...
    // TODO(mcs):
    // 为什么 ChunkList 默认占用一个 Chunk head 的内存
    // ChunkList 又不管理Chunk的生命周期，因为纯管理指针
    struct ChunkList
    {
        Chunk head;
