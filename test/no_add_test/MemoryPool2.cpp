...
struct FreeList
{
    ...
    class ResourcePool
    {
      ...
      public:
        ...
        void append_front(Node *obj) noexcept // NOLINT
        {
            assert(obj != nullptr);
        ...
        void append_back(Node *obj) noexcept // NOLINT
        {
            assert(obj != nullptr);
            assert(obj->prev == nullptr && obj->next == nullptr);
    ...
    ResourcePool free_pool{};
    ResourcePool full_pool{};
    ResourcePool wait_destroy_pool{};
    ...
    void check_shrink() noexcept // NOLINT
    {
        // 缩容操作
        ...
        if (total_count > k_count && !wait_destroy_pool.empty())
        {
            ...
            while (current != nullptr)
            {
                ...
                {
                    wait_destroy_pool.remove(current);
                    total_count -= Chunk::BIT_COUNT;
                    delete current; // 安全删除
                }
    ...
    void sen_back_only(T *ptr) noexcept
    {
        used_count--;
    ...
    void may_change_pool(Chunk *chunk) noexcept // NOLINT
    {
    }
    [[nodiscard]] bool need_full_to_free() const noexcept
    {
        if (total_count < Chunk::BIT_COUNT * 2)
            return true;

        auto ratio = static_cast<double>(used_count) / total_count;
