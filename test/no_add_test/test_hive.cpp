...
struct small_struct
{
    ...
    bool operator==(const small_struct &source) const noexcept
    {
    ...
    {
        {
            ...
            for (hive<int *>::iterator the_iterator = p_hive.begin();
                 the_iterator != p_hive.end(); ++the_iterator)
            {
                ++total;
            ...
            failpass("Iterator + distance test",
                     std::distance(p_hive.begin(), plus_twenty) == 20);
            failpass("Iterator + distance test 2",
                     std::distance(p_hive.begin(), plus_two_hundred) == 200);

            {
                hive<int> d_hive(1000, 1, {20, 20});

                for (hive<int>::iterator current = d_hive.begin(), end = d_hive.end();
                     current != end;)
                {
                    if ((rand() & 7) == 0)
                    {
                ...
                {
                    const int dist1 = rand() % (d_size - 2),
                              dist2 = rand() % ((d_size - 2) - dist1);
                    ...
                    {
                        printf(
                            "positive distance overload fuzz-test failed, real distance "
                            "= %d, reported distance = %d, counter = %d, suite loop = %d",
                ...
                for (int counter = 0; counter != 10000; ++counter)
                {
                    ...
                    if (dist != dist2 - dist1)
                    {
                        printf("positive/negative distance overload fuzz-test failed, "
                               "real distance = %d, reported distance = %d, counter = "
                               "%d, suite loop = %d",
                               dist2, dist, counter, looper);
                        getchar();
                        abort();
            ...
            for (hive<int *>::iterator the_iterator = hive_copy.begin();
                 the_iterator != hive_copy.end(); ++the_iterator)
            {
                ++total;
            }

            failpass("Range-constructor with differing iterator types test",
                     total == 180);

            hive<int *>::iterator next_iterator = std::next(p_hive.begin(), 5);
            hive<int *>::const_iterator prev_iterator = std::prev(p_hive.cend(), 300);

            failpass("Iterator next test",
                     std::distance(p_hive.begin(), next_iterator) == 5);
            failpass("Const iterator prev test",
                     std::distance(prev_iterator, p_hive.cend()) == 300);

            hive<int *>::iterator prev_iterator2 = std::prev(p_hive.end(), 300);
            failpass("Iterator/Const iterator equality operator test",
                     prev_iterator == prev_iterator2);

            prev_iterator = p_hive.begin();
            std::advance(prev_iterator, 5);
            failpass("Iterator/Const iterator equality operator test 2",
                     prev_iterator == next_iterator);
            ...
            for (hive<int *>::reverse_iterator the_iterator = p_hive.rbegin();
                 the_iterator != p_hive.rend(); ++the_iterator)
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            failpass("Reverse iterator advance and distance test",
                     std::distance(p_hive.rbegin(), r_iterator) == 50);

            hive<int *>::reverse_iterator r_iterator2 = std::next(r_iterator, 2);

            failpass("Reverse iterator next and distance test",
                     std::distance(p_hive.rbegin(), r_iterator2) == 52);

            hive<int *>::reverse_iterator r_iterator3 =
                std::make_reverse_iterator(++(p_hive.begin()));

            failpass("std::reverse_iterator and negative distance test",
                     std::distance(r_iterator3, p_hive.rend()) == 1);

            numtotal = 0;
            total = 0;

            for (hive<int *>::iterator the_iterator = p_hive.begin();
                 the_iterator < p_hive.end(); std::advance(the_iterator, 2))
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            for (hive<int *>::const_iterator the_iterator = p_hive.cbegin();
                 the_iterator != p_hive.cend(); ++the_iterator)
            {
                ++total;
                numtotal += **the_iterator;
            }
            ...
            for (hive<int *>::const_reverse_iterator the_iterator =
                     --hive<int *>::const_reverse_iterator(p_hive.crend());
                 the_iterator != hive<int *>::const_reverse_iterator(p_hive.crbegin());
                 --the_iterator)
            {
                ++total;
                numtotal += **the_iterator;
            ...
            for (hive<int *>::iterator the_iterator =
                     ++hive<int *>::iterator(p_hive.begin());
                 the_iterator < p_hive.end(); ++the_iterator)
            {
                ++total;
            ...
            const unsigned int temp_capacity =
                static_cast<unsigned int>(p_hive.capacity());
            ...
            for (hive<int *>::iterator the_iterator =
                     --hive<int *>::iterator(p_hive.end());
                 the_iterator != p_hive.begin(); --the_iterator)
            {
                ++total;
            }
            ...
            for (hive<int *>::iterator the_iterator =
                     --(hive<int *>::iterator(p_hive.end()));
                 the_iterator != p_hive.begin(); std::advance(the_iterator, -2))
            {
                ++total;
            }
...

        {
            ...
            hive<int>::iterator found_item =
                std::find(i_hive.begin(), i_hive.end(), 5000);
            ;

            failpass("std::find iterator test", *found_item == 5000);

            hive<int>::reverse_iterator found_item2 =
                std::find(i_hive.rbegin(), i_hive.rend(), 5000);
            ;

            failpass("std::find reverse_iterator test", *found_item2 == 5000);

            for (hive<int>::iterator the_iterator = i_hive.begin();
                 the_iterator != i_hive.end(); ++the_iterator)
            {
            ...
            failpass("Size after reinitialize + fill-insert test",
                     i_hive.size() == 30000);
            ...
            failpass("Erase randomly till half-empty test",
                     i_hive.size() == 30000u - count2);
            ...
            failpass("Large multi-decrement iterator test 1",
                     end_iterator == end_iterator2);

            for (hive<int>::iterator the_iterator = i_hive.begin();
                 the_iterator != end_iterator;)
            {
                the_iterator = i_hive.erase(the_iterator);
            }
            ...
            for (hive<int>::iterator the_iterator = begin_iterator;
                 the_iterator != end_iterator;)
            {
                the_iterator = i_hive.erase(the_iterator);
            }
            ...
            unsigned int index =
                static_cast<unsigned int>(std::distance(i_hive.begin(), temp_iterator));
            failpass("Advance + iterator-to-index test", index == 20);

            i_hive.erase(temp_iterator);

            failpass("get_iterator test 1",
                     i_hive.get_iterator(&(*temp_iterator)) == i_hive.end());

            temp_iterator = i_hive.begin(); // Check edge-case with advance when erasures
                                            // present in initial group

            failpass("get_iterator test 2",
                     i_hive.get_iterator(&(*temp_iterator)) == temp_iterator);
            ...
            hive<int>::const_iterator temp3 = i_hive.get_iterator(
                const_cast<hive<int>::const_pointer>(&(*temp_iterator)));
            ...
            failpass("get_iterator test 3",
                     i_hive.get_iterator(&(*temp_iterator)) == i_hive.end());

            i_hive.trim_capacity();

            // Test get_iterator etc on a hive with no blocks:
            failpass("get_iterator test 4",
                     i_hive.get_iterator(&(*temp_iterator)) == i_hive.end());

            i_hive.reshape(plf::hive_limits(3, i_hive.block_capacity_limits().max));

            const unsigned int temp_capacity2 =
                static_cast<unsigned int>(i_hive.capacity());
            ...
            failpass("Multiple sequential small insert/erase commands test",
                     count == i_hive.size());
        }

        {
            ...
            std::advance(it1, 400);
            std::advance(it2,
                         500); // This should put it2 past the point of previous erasures
            ...
            std::advance(it1, 4);
            std::advance(it2,
                         9); // This should put it2 past the point of previous erasures
            ...
            std::advance(
                it1,
                345); // Test erasing and validity when it removes the final group in hive
            i_hive.erase(it1, it2);
            ...
            failpass("Range-erase with hive already half-erased, alternating erasures",
                     counter == 904 && i_hive.size() == 904);
            ...
            failpass("Range-erase with hive already third-erased, randomized erasures",
                     counter == 400 && i_hive.size() == 400);
            ...
            for (unsigned int loop_counter = 0; loop_counter != 50; ++loop_counter)
            {
                ...
                while (!i_hive.empty())
                {
                    ...
                    for (hive<int>::iterator it = i_hive.begin(); it != i_hive.end();
                         ++it)
                    {
                        ++counter;
                    }
                    ...
                    {
                        printf("Fuzz-test range-erase randomly Fail: loop counter: %u, "
                               "internal_loop_counter: %u.\n",
            ...
            for (unsigned int loop_counter = 0; loop_counter != 50; ++loop_counter)
            {
                ...
                while (!i_hive.empty())
                {
                    ...
                    {
                        printf(
                            "Fuzz-test range-erase + fill-insert randomly Fails during "
                            "erase: loop counter: %u, internal_loop_counter: %u.\n",
                            loop_counter, internal_loop_counter);
                        getchar();
                        abort();
                    }
                    ...
                    if (i_hive.size() > 100)
                    { // Test to make sure our stored erased_locations are valid &
                      // fill-insert is functioning properly in these scenarios
                        const unsigned int extra_size = rand() & 127;
                        i_hive.insert(extra_size, 5);

                        if (i_hive.size() !=
                            static_cast<unsigned int>(counter) + extra_size)
                        {
                            printf("Fuzz-test range-erase + fill-insert randomly Fails "
                                   "during fill-insert: loop counter: %u, "
                                   "internal_loop_counter: %u.\n",
                                   loop_counter, internal_loop_counter);
                            getchar();
                            abort();
                        ...
                        {
                            printf("Fuzz-test range-erase + fill-insert randomly Fails "
                                   "during counter-test fill-insert: loop counter: %u, "
                                   "internal_loop_counter: %u.\n",
                                   loop_counter, internal_loop_counter);
                            getchar();
                            abort();
            ...
            failpass("Fuzz-test range-erase + fill-insert randomly until empty",
                     i_hive.size() == 0);

            i_hive.erase(i_hive.begin(), i_hive.end());

            failpass("Range-erase when hive is empty test (crash test)",
                     i_hive.size() == 0);
            ...
            failpass("Range-erase when range is empty test (crash test)",
                     i_hive.size() == 10);
        }

        {
            ...
            for (hive<small_struct_non_trivial>::iterator ss_it = ss_nt.begin();
                 ss_it != ss_nt.end(); ++ss_it)
            {
            ...
            failpass("Non-trivial type fuzz-test range-erase randomly until empty",
                     ss_nt.size() == 0);
        }

        {
            ...
            for (hive<int>::iterator current = i_hive.begin(); current != i_hive.end();
                 ++current)
            {
                if (previous > *current)
                {
            ...
            failpass("Less-than sort with allocation optimization 1 test - small types",
                     sorted);
            ...
            for (hive<small_struct>::iterator current = ss_hive.begin();
                 current != ss_hive.end(); ++current)
            {
            ...
            failpass("Less-than sort with allocation optimization 2 test - large type 1",
                     sorted);
            ...
            failpass("Less-than sort with allocation optimization 3 test - large type 2",
                     sorted);
        }

        {
            ...
            std::ranges::take_view<std::ranges::ref_view<plf::hive<int>>> rng =
                i_hive2 | std::ranges::views::take(2);
            ...
            failpass("Range insertion with differing iterators test",
                     i_hive2.size() == 501003);

            i_hive3.insert(std::make_move_iterator(i_hive2.begin()),
                           std::make_move_iterator(i_hive2.cend()));
            ...
            failpass("Reserve + fill insert test",
                     i_hive2.size() == 60000 && total == 60000);
            ...
            failpass("Reserve + fill + fill test",
                     i_hive2.size() == 6060 && total == 6060);
            ...
            failpass("Reserve + fill + fill + reserve + fill test",
                     i_hive2.size() == 12060 && total == 12060);

            i_hive2.clear();
            i_hive2.insert(6000, 2);

            failpass("Clear + fill test",
                     i_hive2.size() == 6000 && *(i_hive2.begin()) == 2);
...

        {
            ...
            failpass("Greater capacity assign test",
                     i_hive.size() == 2000 && total == 40000);

            i_hive.clear();

            for (unsigned int internal_loop_counter = 0; internal_loop_counter != 10;
                 ++internal_loop_counter)
            {
                ...
                {
                    printf("Fuzz-test assign capacity Fail: global loop counter: %u, "
                           "internal loop counter: %u.\n",
                ...
                {
                    printf("Fuzz-test assign sum Fail: global loop counter: %u, internal "
                           "loop counter: %u.\n",
                           looper, internal_loop_counter);
                    getchar();
                    abort();
                 ...
                 ++internal_loop_counter)
            {
                ...
                if (i_hive.size() != capacity)
                {
                    printf("Fuzz-test range assign capacity Fail: global loop counter: "
                           "%u, internal loop counter: %u.\n",
                           looper, internal_loop_counter);
                    getchar();
                    abort();
                }
                ...
                if (i_hive.size() != static_cast<unsigned int>(total))
                {
                    printf("Fuzz-test range assign sum Fail: global loop counter: %u, "
                           "internal loop counter: %u.\n",
                           looper, internal_loop_counter);
                    getchar();
                    abort();
...

        {
            ...
            for (hive<small_struct>::iterator it = ss_hive.begin(); it != ss_hive.end();
                 ++it)
            {
                total2 += it->number;
            }
...

        {
            ...
            failpass("block_capacity_limits test",
                     temp_limits.min == 200 && temp_limits.max == 255);

            temp_limits = plf::hive<int>::block_capacity_hard_limits();

            failpass("block_capacity_hard_limits test",
                     temp_limits.min == 3 && temp_limits.max == 255);
...

        {
            ...
            {
                hive<int> hive1(plf::hive_limits(200, 200)),
                    hive2(plf::hive_limits(200, 200));
...

            {
                ...
                const unsigned int total =
                    static_cast<unsigned int>(hive1.size() + hive2.size());
                ...
                failpass("Large unequal size + erase splice test 1",
                         check_number == total);

                do
                {
                    for (hive<int>::iterator current = hive1.begin();
                         current != hive1.end();)
                    {
                        if ((rand() & 3) == 0)
                        {
                ...
                failpass("Post-splice insert-and-erase randomly till-empty test",
                         hive1.size() == 0);
            }
        }

        {
            ...
            const int count0 =
                static_cast<int>(std::count(i_hive.begin(), i_hive.end(), 0));
            const int count1 = 1000 - count0;

            erase(i_hive, 0);
            failpass("random non-member erase test 1",
                     static_cast<int>(i_hive.size()) == count1);

            erase(i_hive2, 1);
            failpass("random non-member erase test 2",
                     static_cast<int>(i_hive2.size()) == count0);
