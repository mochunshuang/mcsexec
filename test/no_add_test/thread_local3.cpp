...
struct Manager
{
    ...
    struct Chunk
    {
        ...
        MyClass *allocate(uint8_t index, Manager *manager) noexcept
        {
            ...
            new (&block[offset]) BlockHeader{manager, this, index};
    ...
    MyClass *allocate()
    {
        ...
        if (auto chunk = free_list.pop_front())
        {
            ...
            {
                MyClass *obj = chunk->allocate(index, this);
            ...
            free_list.push_front(chunk); // 理论上不可达
        }

        // 扩容：新建Chunk
        Chunk *new_chunk = new Chunk();
        free_list.push_front(new_chunk);
        return new_chunk->allocate(0, this);
    }
    ...
    static void static_deallocate(MyClass *ptr) noexcept
    {
        BlockHeader *header = reinterpret_cast<BlockHeader *>(
            reinterpret_cast<std::byte *>(ptr) - sizeof(BlockHeader));
        header->manager->deallocate(ptr);
    }
};
