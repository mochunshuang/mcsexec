...
struct Promise<Task<T>>
{
    ...
    {
        return Task<T>{std::coroutine_handle<Promise<Task<T>>>::from_promise(*this)};
    }
...
struct [[nodiscard]] Task
{
    ...
    // NOLINTNEXTLINE
    [[nodiscard]] std::coroutine_handle<promise_type> get_handle() const noexcept
    {
        return handle;
    }
  ...
  private:
    ...
    [[nodiscard]] promise_type *get_promise() const noexcept // NOLINT
    {
...
int main()
{
    ...
    assert(task.get_handle().promise().getResult() == 3);
    return 0;
}