...
int main()
{
    ...
    /**
     * @brief 和 counting_scope 类似，但是没有stope_when处理set_error
     * NOTE: 对于 spawn，要求sndr没有任何 set_error 传来，因此需要
     * ex::upon_error([](std::exception_ptr) noexcept {}
     * NOTE:  ex::schedule(pool.get_scheduler()) 带 set_error.要处理
     */
    TEST("base") = [scope = ex::simple_counting_scope{}] mutable {
        ex::sender auto snd =
    ...
    TEST("ctx") = [&] {
        ...
        ex::sender auto snd =
            ...
            ex::upon_error(
                ...
                [](std::exception_ptr e) noexcept { std::rethrow_exception(e); });
        auto start = std::chrono::high_resolution_clock::now();
        try
        {
            // fire, but don't forget
        ...
        struct simple_counting_scope
        {
            struct token
            {
              ...
              private:
                ...
                token(simple_counting_scope *s) : scope(s) {}
    ...
    TEST("with on") = [] {
        ...
        try
        {
            ...
            //   fire, but don't forget
            ex::spawn(
                ex::on(pool2.get_scheduler(), std::move(snd)) |
                    ex::upon_error(
    ...
    TEST("spawn + on with parallel work 2 ") = [] {
        ...
        for (int i = 0; i < num; ++i)
        {
            ...
            ex::spawn(
                ex::on(sch, get_work(i)) | ex::then([&](auto id) noexcept {
    ...
    TEST("spawn + continues_on with parallel work") = [] {
        ...
        {
            ex::spawn(
                get_work(i) | ex::continues_on(sch) |
                    ex::then([&](auto id) noexcept { check_done[id] = true; }) |
                    ex::upon_error(
